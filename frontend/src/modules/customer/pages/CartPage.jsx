...
const CartPage = () => {
    const { cart, loadingCart, cartError, removeItemFromCart, updateItemQuantity } = useCart(); // Changed usage
    const { isAuthenticated } = useAuth(); // Changed usage
    ...
    // Ensure cart object and items array exist before trying to reduce or map them
    if (!cart || !cart.items) return <div className="text-center p-10">Cart not available or empty.</div>;
        ...
        <div className="max-w-3xl mx-auto p-4 sm:p-6 bg-white shadow-xl rounded-lg mt-8">
            ...
            {cart.items.length === 0 ? (
                ...
                <div className="space-y-4">
                    {cart.items.map(item => (
                        <div key={item.id || item.menu_item} className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200 pb-4 mb-4">
                            <div className="flex items-center mb-3 sm:mb-0 flex-grow">
                                ...
                                <div className="flex-grow">
                                    <h3 className="text-lg font-medium text-gray-900">{item.menu_item_details?.name || 'Unknown Item'}</h3>
                                    <p className="text-sm text-gray-500">€{parseFloat(item.menu_item_details?.price || 0).toFixed(2)} each</p>
                                </div>
                            </div>
                            <div className="flex items-center space-x-2 sm:ml-4 self-start sm:self-center">
                                <button onClick={() => updateItemQuantity(item.id || item.menu_item, item.quantity - 1)} disabled={item.quantity <= 1} className="px-2 py-1 border rounded-md text-sm hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">-</button>
                                <span className="px-3 py-1 border-t border-b text-sm w-10 text-center">{item.quantity}</span>
                                <button onClick={() => updateItemQuantity(item.id || item.menu_item, item.quantity + 1)} className="px-2 py-1 border rounded-md text-sm hover:bg-gray-100">+</button>
                                <button onClick={() => removeItemFromCart(item.id || item.menu_item)} className="text-red-500 hover:text-red-700 text-sm ml-2 px-2 py-1 rounded-md hover:bg-red-50">Remove</button>
                            </div>
